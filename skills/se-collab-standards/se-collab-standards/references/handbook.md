...
`// TODO(owner=alice, ticket=PROJ-123): 简短说明`

- `owner` 与 `ticket` 必填，缺一视为违规。
- 说明必须能让接手者 30 秒内理解意图。

### 6.2 实现状态注释（仅半成品必填）

对“只完成一半”的模块或关键文件，必须添加状态块，模板如下：
...
- 安全基线：密钥管理（禁止明文/仓库密钥）、最小权限原则、依赖漏洞扫描（SCA）必须在 CI 中启用。
- 发布与回滚：语义化版本、发布说明（Changelog）、灰度/回滚策略必须可执行。
- 可观测性：关键服务必须有指标（SLI/SLO）、TraceID 贯穿日志与链路追踪，告警分级明确。
- 文档底线：每个服务必须有 README（运行方式/配置）、以及 ADR 记录重大架构决策。

### 7.2 建议尽快补齐 (P1)

- 数据合规：数据保留策略、脱敏规范、备份与恢复演练。
- 依赖治理：依赖升级节奏、弃用策略、供应链安全（SBOM）生成与审计。
